...
test_dates = [[2000, 	1, 	1.5,  	2451545.0],
...
print "{0:^13s}  {1:9s} {2:11s} {3:^8s}" \
      .format("Date", "Actual JD", "Computed JD", "Result")
print (13 * '-') + '  ' + (9 * '-') + " " + (11 * '-') + ' ' + (8 * '-')

# call julian_date for each of the test dates
# and make sure we get the correct result
for test_set in test_dates:
    result = julian_date(test_set[0], test_set[1], test_set[2])

    passed = "PASSED"
    if(result != test_set[3]):
        passed = "*FAILED*"

    format_str = "{0:5d}/{1:02d}/{2:04.1f}  {3:>9.1f} {4:^11.1f} {5:^8s}"
    print str.format(format_str, \
                     test_set[0], \
                     test_set[1], \
                     test_set[2], test_set[3], result, passed)
